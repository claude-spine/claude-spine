...
/**
 * AUDIT: there is currently no safe way to deny. Pick your failure knowingly.
 *
 * This one is about the category of product this tool IS, so it gets stated plainly.
 *
 * #78527 (tehlowkeywiz, 2026-07-17): before 2.1.210, a PreToolUse denial fed back to the model
 * as an is_error tool_result and the model reacted in-turn — 74 denials over thirty versions
 * with 100% turn survival. From 2.1.210 the same denial emits `hook_stopped_continuation` and
 * ENDS THE TURN. No feedback reaches the model, and the Stop-hook chain is skipped, so nothing
 * announces the stall. Unattended sessions and subagents hang until a human arrives.
 *
 * #79449: the obvious mitigation — decide `ask` instead of `deny` — can silently fail to
 * surface, which fails OPEN. The call proceeds and nobody is asked.
 *
 * So both directions are broken, differently, and a guard author is choosing between hanging a
 * session and not guarding at all. That is not a defect in your config and this check cannot
 * fix it. What it can do is make sure you chose, rather than discovering it at 3am.
 *
 * The honest recommendation: `deny` for the genuinely irreversible (a hung turn is recoverable,
 * a dropped table is not), `ask` never for anything unrecoverable, and if you run unattended —
 * CI, subagents, headless — assume a deny may hang the run and alarm on wall-clock elsewhere.
 */
function auditDenyMechanism(repoRoot) {
    ...
    for (const entry of Array.isArray(entries) ? entries : [entries]) {
      for (const h of Array.isArray(entry?.hooks) ? entry.hooks : [entry]) {
        ...
        findings.push({
          event,
          command: h.command,
          type: h.type === 'prompt' ? 'prompt' : 'command',
          why:
            h.type === 'prompt'
              ? `prompt-type PreToolUse hook: denials end the turn on 2.1.210+ (#78527) with the Stop chain skipped, so the stall is silent — and in headless -p mode prompt hooks appear to fail open entirely, returning ok:true even when instructed otherwise. That is enforcement absent in exactly the place you run CI.`
              : `if this hook can deny: on 2.1.210+ a denial ends the turn rather than feeding back (#78527), and the Stop chain is skipped so nothing announces it. Unattended runs hang. The alternative decision, "ask", can silently fail to surface (#79449) and fails OPEN. Choose deliberately: deny for the irreversible, never ask for the unrecoverable.`,
        });
      }
    }
  }
...
/**
 * AUDIT: hooks whose entire product is stdout, on versions where stdout goes nowhere.
 *
 * From #79299 (simpsonbm1, 2026-07-20): SessionStart hook stdout stopped being injected into
 * session context between 2.1.209 and 2.1.215. The hook EXECUTES — proven by its filesystem
 * side effects. Its stdout is NON-EMPTY — verified by re-running the identical command by hand.
 * No `hook_success` attachment is written and the model receives nothing. Same machine, same
 * settings, no config change in between.
 *
 * This is the third distinct shape of the same disease, and the nastiest of the three: the guard
 * is correct, it is invoked, it does its work, and the work is discarded in transit. A canary
 * cannot catch it either — the stamp lands, so liveness reads healthy. Only knowing what the
 * hook was FOR tells you it has stopped doing it.
 *
 * So this is deliberately a heuristic, and it says so: a SessionStart or UserPromptSubmit hook
 * exists almost exclusively to put text in front of the model. If that is what yours is for,
 * it may currently be a no-op on your version, and nothing anywhere will tell you.
 */
function auditContextInjection(repoRoot) {
  const { settings } = resolve(repoRoot);
  const findings = [];
  const INJECTORS = new Set(['SessionStart', 'UserPromptSubmit']);

  for (const [event, entries] of Object.entries(settings.hooks || {})) {
    if (!INJECTORS.has(event)) continue;
    for (const entry of Array.isArray(entries) ? entries : [entries]) {
      for (const h of Array.isArray(entry?.hooks) ? entry.hooks : [entry]) {
        if (typeof h?.command !== 'string') continue;
        findings.push({
          ...
          command: h.command,
          why:
            `${event} hooks exist to put text in front of the model. Between 2.1.209 and 2.1.215 ` +
            `that stdout stopped being injected (#79299) — the hook still runs, its output is still ` +
            `non-empty, the model just never sees it. The canary cannot catch this: the hook fires, ` +
            `so liveness reads healthy. Verify the model actually receives what this prints.`,
        });
      }
    }
  }
  ...
  for (const name of FILES) {
    ...
    for (const [event, entries] of Object.entries(hooks)) {
      ...
      entries.forEach((entry, i) => {
        ...
        // NOT FATAL — and getting this wrong cost me the headline. These two faults shared one
        // branch until a reviewer pulled #75071 and read it against my own prose. The issue is
        // explicit, in the repro notes:
        //
        //   "an invalid-regex string matcher (`"matcher": "*"`) is tolerated — only the
        //    non-string type kills the config."
        //
        // So the file-killer is the TYPE check above (`matcher: {"type":"always"}`), and a bare
        // `*` is not it. Merged, they made the survey count `"*"` as a total outage and produced
        // a 5% "every hook dead" headline the cited source does not support — on a page whose
        // entire argument is that your green check is lying to you. Same species. Mine. In the
        // citation I told the reader to sit with.
        //
        // Still worth reporting: a matcher that cannot compile cannot select a tool, so the
        // entry does nothing. What is no longer claimed is that it takes the file down with it.
        if (typeof entry.matcher === 'string' && entry.matcher !== '') {
          try {
            ...
            faults.push({
              ...
              fatal: false,
              where: `${at}.matcher`,
              what: `will not compile as a regex: ${JSON.stringify(entry.matcher)} — this entry matches nothing`,
              kills: 'this entry only — #75071 reports invalid-regex STRINGS as tolerated; the file-killer is a non-string matcher',
            });
          }
        }
...
module.exports = { resolve, registeredHooks, checkScriptsExist, auditIfConditions, auditSchema, auditDenyRules, auditMcpHooks, auditContextInjection, auditDenyMechanism, FILES };
