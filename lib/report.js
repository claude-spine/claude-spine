...
function print(diag, opts = {}) {
  ...
  if (diag.bare) {
    ...
    L.push('');
    // Third time in one day this early return has swallowed the thing below it — the canary,
    // then the fatal, now the price. The bare path IS the first-run path, so anything that only
    // renders after it is invisible to every new user by construction. Not a coincidence: any
    // block appended to the bottom of print() is dead on arrival here.
    L.push(...closing(diag, opts));
    return L.join('\n');
  }
  ...
  // THE CLOSE. Every price in this project lived in README line 344, which nobody scrolls to.
  // The moment someone is actually persuaded is the four seconds right after this command tells
  // them something true and alarming about their own repo — and until now that moment ended
  // with "run init" and silence. All the machinery, last inch missing, looked complete.
  //
  // Rules it holds itself to, because a pitch that overstates poisons the finding above it:
  //   - only when something was actually found. A clean repo gets nothing.
  //   - the count is real, taken from this run, never a range or a scare number.
  //   - one block, four lines, no second ask. It is a receipt with a price on it.
  L.push(...closing(diag, opts));
  return L.join('\n');
}

function closing(diag, opts = {}) {
  if (opts.quiet || opts.noPitch || process.env.CLAUDE_SPINE_NO_PITCH) return [];

  const fatal = (diag.schema || []).filter((f) => f.fatal).length;
  const found =
    fatal +
    (diag.counts?.MISS || 0) +
    (diag.mcpAudit?.length || 0) +
    (diag.denyAudit?.length || 0) +
    (diag.injectAudit?.length || 0) +
    (diag.ifAudit?.length || 0);

  const unchecked = diag.bare ? diag.results.length : (diag.counts?.absent || 0);
  if (!found && !unchecked) return [];

  const L = [];
  const lead = fatal
    ? `every hook in this file is disabled right now`
    : found
      ? `${found} finding${found > 1 ? 's' : ''} above, none of which your linter reports`
      : `${unchecked} known hazard${unchecked > 1 ? 's' : ''} going unchecked here`;

  L.push(`  ${C.d}${lead}.${C.x}`);
  L.push(
    `  ${C.d}these came from reading all 467 open hook issues on anthropics/claude-code.${C.x}`
  );
  L.push(
    `  ${C.d}new failure modes become checks as they get filed — ${C.x}${C.b}$39/seat/mo${C.x}${C.d}, or $390/yr.${C.x}`
  );
  L.push(`  ${C.b}claude-spine buy${C.x}  ${C.d}· everything you just ran stays free forever${C.x}`);
  L.push('');
  return L;
}
